function [test_RMSE ] = TPCF(R , R_val, RT, d, ind_u_train, ind_v_train, RU, ind_u_RU, ind_v_RU, RI, ind_u_RI, ind_v_RI, RN, ind_u_RN, ind_v_RN, alpha, beta, flag)
test_RMSE = []; 
alpha_u = 0.00; alpha_v = 0.00;
[test_RMSE] = learn(R, R_val, RT, d, ind_u_train, ind_v_train, RU, ind_u_RU, ind_v_RU, RI, ind_u_RI, ind_v_RI, RN, ind_u_RN, ind_v_RN, alpha, beta, flag);
end

function [test_RMSE ] = learn(R, R_val, RT, d, ind_u_train, ind_v_train, RU, ind_u_RU, ind_v_RU, RI, ind_u_RI, ind_v_RI, RN, ind_u_RN, ind_v_RN, alpha, beta, flag)
randn('state', 0);
rand('state', 0);
test_RMSE = [];
n_user = 50000;%max(max(RI(:,1)),max(RU(:,1))) - min(R(:,1)) + 1;
n_item = 5000;%max(max(RU(:,2)),max(RI(:,2))) - min(R(:,2)) + 1;
aux_n_user = 50000;%max(max(RN(:,1)),max(RN(:,1))) - min(RN(:,1)) + 1;
aux_n_item = 5000%max(max(RN(:,2)),max(RN(:,2))) - min(RN(:,2)) + 1;
...
fff = fopen('./3-1_mapping_extreme','a');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
max_iter = 20;
...
for iter = 1 : max_iter
    ...
    prev_rmse = rmse;
    fprintf(fff,'%d,%.5f,%.5f,%d,%.5f,%.5f,%.5f\n',d,alpha,beta,iter,rmse,rmse1,best_val_rmse);
