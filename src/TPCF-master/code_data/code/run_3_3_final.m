...
for a = 1 : numel(alpha)
    for b = 1 : numel(beta)
        n_user = 500%max(train_train(:,1)) - min(train_train(:,1)) + 1
        n_item = 1000%max(train_train(:,2)) - min(train_train(:,2)) + 1
        aux_n_user = 500%max(source(:,1)) - min(source(:,1)) + 1
        aux_n_item = 500%max(source(:,2)) - min(source(:,2)) + 1
        for i = 1 : n_user
            if mod(i,500) == 0
        ...
        flag = 0;
        [rmse ] = TPCF_3_3_final(train_train, train_valid, train_valid, d, ind_u_train , ind_v_train, source, ind_u_source, ind_v_source, alpha(a), beta(b), flag);
        RMSE = [RMSE];
        MAE = [MAE];
    end
end
