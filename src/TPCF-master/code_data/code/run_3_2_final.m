...
RN = source;
RU = load('./md3_2_mapping/RU.shuf');
RI = load('./md3_2_mapping/RI.shuf');
...
for d = dd,
    for a = 1 : numel(alpha)
        for b = 1 : numel(beta)
            ...
            flag = 0;
            [rmse ] =TPCF_3_2_final(train_rating , val_rating,test_rating, d ,ind_u_train , ind_v_train , RU,ind_u_RU,ind_v_RU , RI , ind_u_RI,ind_v_RI , RN,ind_u_RN,ind_v_RN,alpha(a),beta(b),flag);
            RMSE = [RMSE];
            MAE = [MAE];
        end
    end
